    ...
    <template #item="{ item, index }">
      <a-list-item :key="index">
        <a-row>
          ...
          <a-col :span="3">
            <span style="color: #939393">{{ item.mtime }}</span>
          </a-col>
        </a-row>
      </a-list-item>
    </template>
...
<script lang="ts" setup>
  ...
  import { everyDayInfoType, getEveryDayInfoApi } from '@/api/hot';
  import { ref } from 'vue';

  const list = ref<everyDayInfoType[]>([]);
  // 获取数据
  const getData = async () => {
    try {
      const res = await getEveryDayInfoApi();
