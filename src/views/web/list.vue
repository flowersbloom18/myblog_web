<template>
  <div class="container">
    <div class="main">
      ...
      <div class="content">
        <a-card>
          以下是<span style="color: blue">{{ queryName }}</span
          >相关的博客
        </a-card>
        <a-list
...
<script setup lang="ts">
  ...
  import { getTagBlogsApi } from '@/api/tag';
  import { useRoute, useRouter } from 'vue-router';
  import { getCategoryBlogsApi } from '@/api/category';

  const route = useRoute();

  // 查询类型，是标签还是分类
  const paramType = ref(route.params.type as string);
  ...
  const getData = async () => {
    if (paramType.value === 'tag') {
      const { data } = await getTagBlogsApi(queryName.value);
      // 只导出发布的
      dataList.value = data.data.list.filter((item: any) => item.is_publish);

      // dataList.value = data.data.list;
    } else if (paramType.value === 'category') {
      const { data } = await getCategoryBlogsApi(queryName.value);
      // 只导出发布的
      dataList.value = data.data.list.filter((item: any) => item.is_publish);
