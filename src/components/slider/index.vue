<template>
  <div class="result">
    <!--滑动验证码，在原基础上进行修改，修复了本地图片验证的方式，图片存储为数组-->
  ...
  export default {
    ...
    props: {
      ...
      imgUrls: {
        type: Array,
        // default: () => [
        //   'https://picsum.photos/300/150/?image=1',
        //   // 添加更多图片 URL
        // ],
      },
    },
    emits: ['refresh', 'success', 'fail'],
    data() {
      return {
    ...
    methods: {
      ...
      getRandomImg() {
        ...
        // 优化方案2 使用本地图片
        // 获取图片数组中0-（length-1）的数据
        const lens = this.imgUrls.length;
        const num = Math.floor(Math.random() * lens);
